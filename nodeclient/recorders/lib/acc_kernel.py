...
class adxl355z:
    ...
    LOGGER:Logger
    acc_kernel_file_exist = os.path.exists("/sys/devices/platform/morpheusbox_acc_v1/")


    def __init__(self, channel, logger:Logger=None):
        self.LOGGER = logger

        self.acc_x_file_exist = os.path.exists("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_xdata")
        self.acc_y_file_exist = os.path.exists("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_ydata")
        self.acc_z_file_exist = os.path.exists("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_zdata")
        self.acc_time_file_exist = os.path.exists("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_time")

        if self.acc_x_file_exist:
            self.x_path = "/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_xdata"
            #self.acc_xfile = open("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_xdata", "r")
        if self.acc_y_file_exist:
            self.y_path = "/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_ydata"
            #self.acc_yfile = open("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_ydata", "r")
        if self.acc_z_file_exist:
            self.z_path = "/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_zdata"
            #self.acc_zfile = open("/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_zdata", "r")
        if self.acc_time_file_exist:
            self.time_file = "/sys/devices/platform/morpheusbox_acc_v1/acc"+str(channel)+"_time"
