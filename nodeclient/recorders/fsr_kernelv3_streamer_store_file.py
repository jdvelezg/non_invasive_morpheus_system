import sys
import time
from lib.fsr_kernelv3 import ads1115_fsr
...
def dataline_to_datarecord(dataline, timeline, recordingId, sensorID, label='raw'):
    """Convert the dataline extracted from kernel file into a datarecord."""
    
    datarecord = DataRecord(label=label)

    kernel_datalist = dataline.split(",")
    data_items = len(kernel_datalist)
    kernel_timelist = timeline.split(",")

    end_epoch = int(kernel_timelist[data_items-1])
    init_epoch = int(kernel_timelist[0])
    ...
    datarecord.timeMap = kernel_timelist
...
if __name__ == "__main__":
    ...
    filepath = arg_values[path] + "/" + arg_values[recording_uuid] + "_" + arg_values[sensor_uuid] + "_rawFSR" + ".json"
    #reset
    ...
    topic = "/" +arg_values[topic] +"/" +arg_values[sensor_uuid]+"/raw_FSR"
    LOGGER.info("sending to topic: "+topic)

    #Recorder
    recorder = ads1115_fsr(arg_values[channel], LOGGER)
    ...
    with open(filepath, 'a') as file:
        try:
            ...
            while True:
                ...
                val, timestamps = recorder.read_register()
                data_record = dataline_to_datarecord(val,timestamps,arg_values[recording_uuid],arg_values[sensor_uuid])
